...
async function migrateFirebaseUsers() {
  ...
  for (const r of rows) {
    ...
    const existing = await prisma.firebaseUser.findFirst({ where: { userId } });
...
async function migratePlans() {
  ...
  for (const p of vsPlans) {
    ...
    let permAccess = null,
      ...
      subPlanId = null;
    try {
      permAccess = JSON.parse(p.permission_access);
    } catch {}
    try {
      benefits = JSON.parse(p.benefits);
    } catch {}
    try {
      subPlanId = JSON.parse(p.sub_plan_id);
    } catch {}
...
async function migrateShapes(pool, source, appType) {
  ...
  for (const s of rows) {
    const groupId = s.group_id
      ? groupIdMap.get(mapKey(s.group_id, source))
      : null;
