...
exports.getConsent = asyncHandler(async (req, res) => {
  ...
  const result = await aiAssistantService.getConsent(req.user.id, teamId);
  res.json({ success: true, data: result });
});
...
exports.updateConversationTitle = asyncHandler(async (req, res) => {
  const { title } = req.body || {};
  const result = await aiAssistantService.updateConversationTitle(
    req.user.id,
    req.params.conversationId,
    title,
  );
...
exports.analyzeDocument = asyncHandler(async (req, res) => {
  ...
  if (mimeType === "application/pdf") {
    try {
      ...
      return res.status(400).json({
        success: false,
        error: { code: "PARSE_ERROR", message: "Could not extract PDF text" },
      });
    }
    ...
    try {
      ...
      return res.status(400).json({
        success: false,
        error: { code: "PARSE_ERROR", message: "Could not extract DOCX text" },
      });
    }
  } else {
...
exports.generateDiagramFromDocument = asyncHandler(async (req, res) => {
  ...
  const result = await aiAssistantService.generateDiagramFromDocument(
    req.user.id,
    documentText,
    fileName,
  );
