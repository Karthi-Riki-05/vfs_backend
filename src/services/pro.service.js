...
class ProService {
  ...
  async switchApp(userId, app) {
    ...
    if (app === "pro") {
      ...
      if (!user.hasPro) {
        throw new AppError(
          "No Pro access. Purchase Pro to access this app.",
          403,
          "PRO_REQUIRED",
        );
      }
  ...
  async createProPurchaseCheckout(userId) {
    ...
    const proPriceId = process.env.STRIPE_PRO_MONTHLY_PRICE;
    const hasProPrice = proPriceId && proPriceId !== "placeholder";

    const sessionConfig = {
      mode: hasProPrice ? "subscription" : "payment",
      customer: customerId,
      line_items: [
        hasProPrice
          ...
          : {
              price_data: {
                ...
                unit_amount: 100, // $1.00 legacy fallback
              },
              quantity: 1,
            },
      ],
    ...
    if (hasProPrice) {
      sessionConfig.subscription_data = {
        metadata: { userId, purchaseType: "pro_upgrade" },
      };
    }
