...
class AdminService {
  ...
  async getTransactions(options = {}) {
    const { status, appContext, page = 1, limit = 20 } = options;
    const take = Math.min(Number(limit) || 20, 100);
    const skip = (Math.max(Number(page) || 1, 1) - 1) * take;

    const where = {};
    if (status) where.status = status;
    // Optional appContext filter so admin can isolate Free / Pro / Team
    // transactions; omit to see everything (default).
