...
class SubscriptionService {
    ...
    _getPriceId(plan) {
        ...
        if (plan === 'yearly') return process.env.STRIPE_YEARLY_PRICE_ID || null;
        return null;
    }
    ...
    async createCheckoutSession(userId, { plan, teamMembers }) {
        ...
        if (existingSub && existingSub.status === 'active' && existingSub.paymentId) {
            throw new AppError('You already have an active subscription. Use change plan instead.', 400, 'ALREADY_SUBSCRIBED');
        }
        ...
        const lineItem = priceId
            ...
            : {
                price_data: {
                    ...
                    product_data: process.env.STRIPE_PRODUCT_ID ? undefined : {
    ...
    async getStatus(userId) {
        ...
        if (subscription.productType) {
            planType = subscription.productType.includes('yearly') ? 'yearly' : 'monthly';
    ...
    async cancelSubscription(userId) {
        ...
        if (!subscription.paymentId) throw new AppError('No Stripe subscription to cancel', 400, 'NO_STRIPE_SUB');
    ...
    async changePlan(userId, { plan, teamMembers }) {
        ...
        if (subscription.productType) {
            currentPlan = subscription.productType.includes('yearly') ? 'yearly' : 'monthly';
        }
        ...
        if (currentPlan === plan) {
            ...
            logger.info(`Member count changed for user ${userId}: ${plan}, ${teamMembers} members`);
            return { type: 'updated', message: 'Team member count updated successfully' };
        }
        ...
        if (currentPlan === 'monthly' && plan === 'yearly') {
            ...
            logger.info(`Plan change scheduled for user ${userId}: monthly → yearly at ${activationDate.toISOString()}`);
            return {
                type: 'scheduled',
    ...
    async activateScheduledPlan(userId) {
        ...
        if (!subscription) throw new AppError('No subscription found', 404, 'NOT_FOUND');
        if (!subscription.scheduledPlanType) {
            throw new AppError('No scheduled plan change found', 400, 'NO_SCHEDULED_CHANGE');
        }
        ...
        if (subscription.paymentId) {
            try {
                ...
                logger.warn(`Failed to cancel old Stripe sub ${subscription.paymentId}: ${err.message}`);
            }
        }
        ...
        const result = await this.createCheckoutSession(userId, { plan, teamMembers });

        logger.info(`Scheduled plan activated for user ${userId}: creating checkout for ${plan}`);
        return result;
    }
    ...
    async handleWebhook(rawBody, signature) {
        ...
        logger.info(`Body type: ${typeof rawBody}, isBuffer: ${Buffer.isBuffer(rawBody)}`);
        ...
        logger.info(`Stripe subscription webhook verified: ${event.type}`, { eventId: event.id });
    ...
    async _handleCheckoutComplete(session) {
        const { userId, plan, teamMembers } = session.metadata || {};
        logger.info('=== CHECKOUT SESSION COMPLETED ===');
        logger.info(`Session ID: ${session.id}, Customer: ${session.customer}`);
        logger.info(`Subscription: ${session.subscription}, Metadata: ${JSON.stringify(session.metadata)}`);

        if (!userId || !plan) {
            logger.error('Missing userId or plan in session metadata — cannot save subscription');
            return;
        }
        ...
        if (!dbPlan) {
            dbPlan = await prisma.plan.create({
                data: {
                    ...
                    userAccess: true,
                    userCount: members,
                    features: JSON.stringify(['Unlimited flows', 'All shapes', 'Export all formats', 'Team collaboration', 'Admin dashboard', 'Team management', 'Priority support', 'AI diagram generation']),
                },
            });
        }
        ...
        logger.info(`Subscription activated for user ${userId}: ${plan}, ${members} members`);
    }
    ...
    async _handleInvoicePaid(invoice) {
        ...
        if (!sub) {
            logger.warn(`No subscription found for paymentId: ${invoice.subscription}`);
            return;
        }

        const expiresAt = new Date();
        if (sub.productType === 'team_yearly' || sub.productType === 'pro_yearly') {
            expiresAt.setFullYear(expiresAt.getFullYear() + 1);
        } else {
            expiresAt.setMonth(expiresAt.getMonth() + 1);
        }
        ...
        logger.info(`Subscription renewed for user ${sub.userId}. New expiry: ${expiresAt.toISOString()}`);
    }

    async _handlePaymentFailed(invoice) {
        logger.warn(`=== PAYMENT FAILED === invoice: ${invoice.id}, subscription: ${invoice.subscription}`);
        if (!invoice.subscription) return;
        ...
        if (sub) {
            ...
            logger.warn(`Subscription ${sub.id} marked as past_due for user ${sub.userId}`);
        }
    }
    ...
    async reactivateSubscription(userId) {
        ...
        const subscription = await prisma.subscription.findUnique({ where: { userId } });
        if (!subscription) throw new AppError('No subscription found', 404, 'NOT_FOUND');
        if (!subscription.paymentId) throw new AppError('No Stripe subscription to reactivate', 400, 'NO_STRIPE_SUB');
        if (subscription.status !== 'cancelling') {
            throw new AppError('Subscription is not in cancelling state', 400, 'NOT_CANCELLING');
        }
    ...
    async verifySession(userId, sessionId) {
        ...
        logger.info(`Verify session ${sessionId}: status=${session.status}, payment=${session.payment_status}`);
        ...
        if (existing && existing.status === 'active' && existing.paymentId) {
            logger.info(`Subscription already exists for user ${userId}, returning status`);
        ...
        if (!plan) {
            throw new AppError('Missing plan in session metadata', 400, 'INVALID_SESSION');
        }
